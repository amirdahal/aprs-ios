...
void radioEventsHandler(RadioEvents eventType, dynamic data) {
    switch (eventType) {
      ...
      case RadioEvents.radioSettingChanged:
        if (kDebugMode) {
          print("Radio setting updated: ${RadioExtract.radio.radioSetting.toMap()}");
        }
        break;
      case RadioEvents.aprsSettingChanged:
        if (kDebugMode) {
          print("Aprs setting updated: ${RadioExtract.radio.aprsSetting.toMap()}");
        }
        break;
      case RadioEvents.radioStatusChanged:
        Status status = data;
        if (kDebugMode) {
          print("Radio status changed: ${status.toMap()}");
        }
        break;
    }
  }